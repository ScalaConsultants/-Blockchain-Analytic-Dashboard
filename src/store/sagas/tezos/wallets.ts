...
async function fetchWallets( {limit = 10, groupBy = 'buyer', from = 1567296000, to = 1567382400} ): Promise<Wallets> {
  let res = null;
  if(groupBy == 'data') {
    res = await fetch(`${process.env.REACT_APP_HOST}/api/v1/tezos/data-wallets/cache?limit=${limit}&from=${from}&to=${to}`);
  } else {
    res = await fetch(`${process.env.REACT_APP_HOST}/api/v1/tezos/wallets/cache?groupBy=${groupBy}&limit=${limit}&from=${from}&to=${to}`);
  }
