...
async function fetchTransactions(walletHash:string, page:number): Promise<Transactions> {
  const res = await fetch(
    `${process.env.REACT_APP_HOST}/api/v1/ethereum/transactions?limit=100&walletHash=${walletHash}`
  );
  return res.json();
}
