...
async function fetchWallets( {limit = 10, groupBy = 'buyer', from = 1567296000, to = 1567382400} ): Promise<Wallets> {
  let res = null;
  if(groupBy == 'data') {
    res = await fetch(`${process.env.REACT_APP_HOST}/api/v1/ethereum/data-wallets/cache?limit=${limit}&from=${new Date().getTime() - 1000 * 3600 * 24 * 7}&to=${new Date().getTime()}`);
  } else {
    res = await fetch(`${process.env.REACT_APP_HOST}/api/v1/ethereum/wallets/cache?groupBy=${groupBy}&limit=${limit}&from=${new Date().getTime() - 1000 * 3600 * 24 * 7}&to=${new Date().getTime()}`);
  }
